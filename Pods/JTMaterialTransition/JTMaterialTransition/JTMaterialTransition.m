...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    if(!self.isReverse){
        ...
        expandAnim.toValue = [NSValue valueWithCATransform3D:CATransform3DMakeScale(finalTransform.a, finalTransform.d, 1)];
        expandAnim.timingFunction = [CAMediaTimingFunction functionWithControlPoints:0.95 :0.02 :1 :0.05];
        expandAnim.duration = [self transitionDuration:transitionContext] * .7;
        expandAnim.fillMode = kCAFillModeForwards;
        expandAnim.removedOnCompletion = YES;
        [animatedViewForTransition.layer addAnimation:expandAnim forKey:expandAnim.keyPath];
        
        CABasicAnimation *position = [CABasicAnimation animationWithKeyPath:@"position"];
        position.toValue = [NSValue valueWithCGPoint:transitionContext.containerView.center];
        position.timingFunction = [CAMediaTimingFunction functionWithControlPoints:0.95 :0.02 :1 :0.05];
        position.duration = [self transitionDuration:transitionContext] * .7;
        ...
        backgroundColor.toValue = presentedController.view.backgroundColor;
        backgroundColor.timingFunction = [CAMediaTimingFunction functionWithControlPoints:0.95 :0.02 :1 :0.05];
        backgroundColor.duration = [self transitionDuration:transitionContext] * .7;
